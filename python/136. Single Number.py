...
Find that single one. Find a solution with O(N) time and O(N) space.
...
which appears exactly once. Find the single element with O(N) time and O(1) space.
...
class Solution:
    def singleNumber(self, nums: List[int]) -> int:
        """
        Define frequency table to count the occurrence of numbers and
        return the one which only appears once.
        This is not the most efficient but is extremely practical.
        Time: O(N), Space: O(N)
        """
...
Given an integer array nums, in which exactly two elements appear only once and all the other elements appear exactly twice. Find the two elements that appear only once. You can return the answer in any order. Find a solution in O(N) time and O(1) space.
...
Given an array of integers nums containing n + 1 integers where each integer is in the range [1, n] inclusive. There is only one repeated number in nums, return this repeated number. Find a solution in O(N) time and O(1) space.

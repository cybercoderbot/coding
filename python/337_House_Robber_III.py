...
class Solution(object):
    ...
    def dfs(self, root, visited):
        ...
        val = 0
        if root.left:
            val += self.dfs(root.left.left, visited) + \
                self.dfs(root.left.right, visited)
        if root.right:
            val += self.dfs(root.right.left, visited) + \
                self.dfs(root.right.right, visited)
        val = max(val + root.val, self.dfs(root.left, visited) +

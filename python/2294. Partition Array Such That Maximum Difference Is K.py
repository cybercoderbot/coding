...
Iterate each element nums[i] in the input array,
...
nums[i] cannot be added to the subsequence,
so we start a new subsequence with nums[i] as the first element,
...
class Solution:
    def partitionArray(self, nums: List[int], k: int) -> int:
        """
        Sort nums
        Keep running min and max of the array
        if mx - mn > k: res += 1, mx = mn = x
        """
        nums.sort()
        res = 1
        mn = mx = nums[0]

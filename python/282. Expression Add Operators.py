    ...
    def addOperators(self, s: str, target: int) -> List[str]:
        ...
        def backtrack(i, expr, cursum, last):
            ...
            for j in range(i, N):
                ...
                num = int(s[i:j+1])
                if i == 0:
                    backtrack(j+1, str(num), num, num)  # First num, pick it without adding any operator
                else:
                    backtrack(j+1, expr + "+" + str(num), cursum + num, num)
                    backtrack(j+1, expr + "-" + str(num), cursum - num, -num)
                    backtrack(j+1, expr + "*" + str(num), cursum - last + last * num, last * num)  # Can imagine with example: 1+2*3*4
